...
function makeToken(){const exp=Date.now()+12*60*60*1000;const data=`admin.${exp}`;return `${data}.${sign(data)}`}
function isAdmin(req){const t=parseCookies(req).featera_session;if(!t)return false;const p=t.split('.');if(p.length!==3)return false;const data=`${p[0]}.${p[1]}`;const a=Buffer.from(sign(data)),b=Buffer.from(p[2]);if(a.length!==b.length||!crypto.timingSafeEqual(a,b))return false;return p[0]==='admin'&&Number(p[1])>Date.now()}
function safeEqual(a,b){const x=Buffer.from(String(a)),y=Buffer.from(String(b));return x.length===y.length&&crypto.timingSafeEqual(x,y)}

app.post('/api/login',(req,res)=>{
  const {user='',pass=''}=req.body||{};
  if(String(user).toLowerCase()===String(ADMIN_USER).toLowerCase()&&safeEqual(pass,ADMIN_PASSWORD)){
    res.setHeader('Set-Cookie',`featera_session=${makeToken()}; Path=/; HttpOnly; SameSite=Lax; Max-Age=43200${process.env.RAILWAY_ENVIRONMENT?'; Secure':''}`);
...
async function init(){
  ...
  app.listen(PORT,'0.0.0.0',()=>console.log(`FEATERA Calendar PostgreSQL edition running on port ${PORT}`));
}
