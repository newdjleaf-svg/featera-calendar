...
function renderCalendar(){
  ...
  for(let i=0;i<weeks*7;i++){
    ...
    html+=`<div class="day in-month ${dow===0||dow===6?'weekend':''}" data-date="${date}"><div class="date-strip">${pad(d)}</div><div class="day-content">${evs.map(renderEvent).join('')}</div></div>`;
  }
...
function openEventEditor(id,date){
  ...
  openModal(e?'編輯行程':'新增行程',`<div class="form-grid">
    ...
    <label class="field"><span>第一行底色</span><select id="evHighlight"><option value="1" ${obj.highlight?'selected':''}>米黃色</option><option value="0" ${!obj.highlight?'selected':''}>無</option></select></label>
